...
for i = 1 : length(time)
    ...
    [XS_min(:,:,i), dtS_min(:,i)] = sp3_linear('igu18893_18.sp3', k_min, date(i,:)); 
    ...
    [XS_max(:,:,i), dtS_max(:,i)] = sp3_linear('igu18893_18.sp3', k_max, date(i,:));
...
mkdir(strcat('./data/', 'DG_PP_linear', time_stamp));
pathname = strcat('./data/', 'DG_PP_linear', time_stamp, '/');
save(strcat(pathname,'PP_linear_DG_XS_min_', time_stamp), 'XS_min');
save(strcat(pathname,'PP_linear_DG_XS_max_', time_stamp), 'XS_max');
save(strcat(pathname,'PP_linear_DG_XR_min_', time_stamp), 'XR_min');
save(strcat(pathname,'PP_linear_DG_XR_max_', time_stamp), 'XR_max');
save(strcat(pathname,'PP_linear_DG_time_', time_stamp), 'time');
save(strcat(pathname,'PP_linear_DG_dtR_min_', time_stamp), 'dtR_min');
save(strcat(pathname,'PP_linear_DG_dtR_max_', time_stamp), 'dtR_max');
save(strcat(pathname,'PP_linear_DG_dtS_min_', time_stamp), 'dtS_min');
save(strcat(pathname,'PP_linear_DG_dtS_max_', time_stamp), 'dtS_max');
save(strcat(pathname,'PP_linear_DG_pr1_', time_stamp), 'pr1');
