...
for i = 1 : length(time)
    ...
    [XS, dtS] = sp3_linear('igu18893_18.sp3', index, date(i,:));
    ...
    while(var_SPP(1) > SPP_threshold^2 && n_iter < n_iter_max)
        [XR, dtR, cov_XR, var_dtR, PDOP, HDOP, VDOP, cond_num, bad_obs, bad_epoch, var_SPP] = LS_SA_code2(XR0, XS, pr1(index,i), zeros(nsat_avail,1), zeros(nsat_avail,1), zeros(nsat_avail,1), dtS, zeros(nsat_avail,1), zeros(nsat_avail,1), [], SPP_threshold);       
