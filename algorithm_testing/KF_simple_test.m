...
for i = 1:time_steps
    [XR1(:,i), XR_cov1(:,:,i), dtR1(i)] = KF_simple(XS(:,:,i), pr(:,i));
%     [XR1(:,i), dtR1(i)] = tikhonov(XS(:,:,i), pr(:,i));
    %[XR2(:,i), dtR2(i)] = tikhonov2(XS(:,:,i), pr(:,i));
    %d1(i) = sqrt((XR1(:,i) - pos(1))'*(XR1(:,i) - pos(1)));
    %[XR2(:,i), XR_cov2(:,:,i), dtR2(i)] = DG4sat(XS(:,:,i), pr(:,i));
end


    [XR2(:,i), XR_cov2(:,:,i), dtR2(i)] = DG_sol(XS(:,:,i), pr(:,i));
    %d2(i) = sqrt((XR2(:,i) - pos(2))'*(XR2(:,i) - pos(2)));
    [XR3(:,i), XR_cov3(:,:,i), dtR3(i)] = leastSquare2(XS(:,:,i), pr(:,i));
    %d3(i) = sqrt((XR3(:,i) - pos(3))'*(XR3(:,i) - pos(3)));
end
